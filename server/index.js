...
const stripe = require('stripe')(`sk_test_51R2vykRpuWp8AHsRt2Rw8GVbw55RJwvnBXCF3iJVI1uVe5umJ6uB4hFCGMIa2QqAIIq1ofC9T37yGySrTs2phhuk00UEbHmWUJ`)
...
const verifyToken = async (req, res, next) => {
  ...
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, decoded) => {
    ...
    req.user = decoded /* from here (ref:01) */
    next()
  })
}
...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.oavir.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    const verifyAdmin = async (req, res, next) => {
      const user = req.user; /* req.user isSetFromVerifyToken (ref01) */
      const query = { email: user?.email }
      const result = await usersCollection.findOne(query)
      if (!result || result?.role !== 'admin') return res.status(401).send({ message: 'Unauthorized admin Access!!!' })
      next()
    }
    // Verify Admin Middleware 
    const verifyHost = async (req, res, next) => {
      ...
      const query = { email: user?.email }
      const result = await usersCollection.findOne(query)
      if (!result || result?.role !== 'host') return res.status(401).send({ message: 'Unauthorized host Access!!!' })
      next()
    }
...
//     // Get a single room data from db using _id
  ...
  /* Checking if i am connected with mongodb successfully */
  // try {
  //   await client.connect() // Connect to MongoDB explicitly
  //   console.log('Connected to MongoDB successfully!')

  //   // Check authentication by listing databases
